        ...
        <>
          <section className="mb-8">
            ...
            {accounts.length === 0 ? (
              ...
              <div className="space-y-4">
                ...
                <ul className="divide-y divide-gray-200 rounded-lg border border-gray-200 dark:divide-slate-700 dark:border-slate-700">
                  {accounts.map((acc) => (
                    <li key={acc.id} className="flex items-center justify-between px-4 py-3">
                      <div>
                        ...
                        {acc.site_id && (
                          <span className="ml-2 text-sm text-gray-500 dark:text-slate-400">({acc.site_id})</span>
                        )}
                      </div>
          ...
          {accounts.length > 0 && (
            <section className="mb-8">
              ...
              {reputationLoading ? (
                <p className="text-gray-500 dark:text-slate-400">Carregando reputação…</p>
              ) : !reputation ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
                      <p className="text-sm text-gray-500 dark:text-slate-300">Taxa de Reclamações</p>
                      <p
                        className={`mt-1 text-2xl font-bold ${
                          reputation.reputation.metrics.claims.rate <= 0.02
                            ? "text-green-600"
                            : reputation.reputation.metrics.claims.rate <= 0.045
                            ? "text-yellow-600"
                            : reputation.reputation.metrics.claims.rate <= 0.08
                            ? "text-orange-600"
                            : "text-red-600"
                        }`}
                      >
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
                      <p className="text-sm text-gray-500 dark:text-slate-300">Atrasos no Envio</p>
                      <p
                        className={`mt-1 text-2xl font-bold ${
                          reputation.reputation.metrics.delayed_handling_time.rate <= 0.10
                            ? "text-green-600"
                            : reputation.reputation.metrics.delayed_handling_time.rate <= 0.18
                            ? "text-yellow-600"
                            : reputation.reputation.metrics.delayed_handling_time.rate <= 0.22
                            ? "text-orange-600"
                            : "text-red-600"
                        }`}
                      >
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
                      <p className="text-sm text-gray-500 dark:text-slate-300">Taxa de Cancelamentos</p>
                      <p
                        className={`mt-1 text-2xl font-bold ${
                          reputation.reputation.metrics.cancellations.rate <= 0.015
                            ? "text-green-600"
                            : reputation.reputation.metrics.cancellations.rate <= 0.035
                            ? "text-yellow-600"
                            : reputation.reputation.metrics.cancellations.rate <= 0.04
                            ? "text-orange-600"
                            : "text-red-600"
                        }`}
                      >
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        <div className="mb-1 flex justify-between text-xs">
                          ...
                          <span className="font-medium">
                            {formatPercent(reputation.reputation.transactions.ratings.positive)}
                          </span>
                        </div>
                      ...
                      <div className="flex-1">
                        <div className="mb-1 flex justify-between text-xs">
                          <span className="text-gray-500 dark:text-slate-300">Neutras</span>
                          <span className="font-medium">
                            {formatPercent(reputation.reputation.transactions.ratings.neutral)}
                          </span>
                        </div>
                        <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-slate-700">
                          <div
                      ...
                      <div className="flex-1">
                        <div className="mb-1 flex justify-between text-xs">
                          ...
                          <span className="font-medium">
                            {formatPercent(reputation.reputation.transactions.ratings.negative)}
                          </span>
                        </div>
                        <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-slate-700">
                          <div
                    ...
                    <p className="mt-3 text-xs text-gray-500 dark:text-slate-400">
                      ...
                      ({reputation.reputation.transactions.completed} concluídas,{" "}
                      {reputation.reputation.transactions.canceled} canceladas)
                    </p>
                  </div>
                  ...
                  <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-900/50 dark:bg-blue-950/40">
                    ...
                    <ul className="mt-2 grid gap-2 text-xs text-blue-700 dark:text-blue-200 sm:grid-cols-3">
                      <li>Reclamações: ≤ 1%</li>
      ...
      {activeTab === "frete" && (
        <section className="mb-8">
          ...
          <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-900/50 dark:bg-blue-950/40">
            ...
            <ul className="mt-2 list-inside list-disc text-sm text-blue-700 dark:text-blue-200">
          ...
          {shippingLoading ? (
            <p className="text-gray-500 dark:text-slate-400">Carregando tabela de frete…</p>
        ...
        <section className="mb-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <button
              ...
              className={`flex flex-col items-start gap-2 rounded-lg border p-4 text-left ${
                theme === "dark"
                  ? "border-blue-500 bg-slate-900 dark:border-blue-400 dark:bg-slate-900"
          ...
          <p className="mt-4 text-xs text-gray-500 dark:text-slate-400">
            Sua escolha é salva neste navegador e aplicada em todas as telas do aplicativo.
          </p>
        </section>
      )}
    </div>
